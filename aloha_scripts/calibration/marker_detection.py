...
def estimate_transformation(corners, ids, camera_matrix, dist_coeffs):
    ...
    if ids is not None and len(ids) > 0:
        ...
        rvecs, tvecs, _ = estimatePoseSingleMarkers(corners, marker_size, camera_matrix, dist_coeffs)
...
def camera2marker():
    ...
    if ir_frame is not None:
        ...
        if ids is not None and len(ids) > 0:
            print(f"{len(ids)} aruco markers detected")
            result = {}
            for i in range(len(ids)):
                transform_matrix = estimate_transformation(corners[i], ids[i], camera_matrix, dist_coeffs)
                if transform_matrix is not None:
                    result[ids[i][0]] = transform_matrix
            print(result)
            return result
